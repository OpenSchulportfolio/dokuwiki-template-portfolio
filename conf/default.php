<?php

// portfolio title
$conf["sitetitle"]    = "Schulportfolio"; //TRUE: use/show user pages
$conf["schoolname"]    = "Schulname hier eintragen"; //TRUE: use/show user pages
$conf["barcolor"]    = "#FFE28A";
//site notice
$conf["sitenotice"]          = true; //TRUE: use/show sitenotice
$conf["sitenotice_location"] = ":allusers:topmenu"; //page/article used to store the sitenotice
$conf["menuconf_sepchar"]    = ":::";

//$conf["infomail"]    = array("onoff");

//discussion pages
//$conf["discussion"]    = array("onoff");
//$conf["discussion_namespace"] = array("string", "_pattern" => "/^:.{1,}:$/");

//site notice
//$conf["vector_sitenotice"]          = array("onoff");
//$conf["vector_sitenotice_location"] = array("string");


// Winmuster
$conf["winML_logout"]   = false; //Logout link according to WinMl SSO?
$conf["winML_logout_argument"] = "CMD=logoff"; // String to attach to url for logging out
$conf["winML_hide_loginlogout"] = false; // Hide login/logout functions
$conf["winML_hide_loginlogout_subnet"] = "10.1.x.x"; // wehn hiding, for wicht subnets?
$conf["closedwiki"]            = true; //TRUE: hides most tabs/functions until user is logged in

